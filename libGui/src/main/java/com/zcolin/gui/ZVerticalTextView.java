 ...
 */
public class ZVerticalTextView extends AppCompatTextView {

    /** 是否将文字显示成竖排 */
    private boolean mIsVerticalMode = true;
    /** 行数 */
    private int     mLineCount;
    /** 下标: 行号; 数组内容: 该行的宽度(由该行最宽的字符决定) */
    private float[] mLineWidths;
    /** 下标: 行号; 数组内容: 该行最后一个字符的下标 */
    private int[]   mLineBreakIndex;

    public ZVerticalTextView(Context context) {
        super(context);
        init();
    }

    public ZVerticalTextView(Context context, AttributeSet attrs) {
        super(context, attrs);
        init();
    }

    public ZVerticalTextView(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (mIsVerticalMode) {
            ...
            final int lineMaxBottom = (heightMode == MeasureSpec.UNSPECIFIED ?
                                       Integer.MAX_VALUE :
            ...
            int lineIndex = 0;
            mLineCount = 0;
            // 加1是为了处理高度不够放下一个字的情况,needBreakLine会一直为true直到最后一个字
            mLineWidths = new float[chars.length + 1];
            ...
            for (int i = 0; i < chars.length; i++) {
                ...
                // is need break line
                boolean needBreakLine = currentLineHeight + charHeight > lineMaxBottom && i > 0;
                // 是因为即使在第一列高度不够,也不用换下一列
                if (needBreakLine) {
